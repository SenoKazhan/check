...
async def update_setting(
    ...
    if expected_type == "float" and isinstance(value, (int, float)):
        field_info = getattr(settings.__fields__.get(
            key), "field_info", None) if hasattr(settings, "__fields__") else None
        if field_info:
            if hasattr(field_info, "ge") and field_info.ge is not None and value < field_info.ge:
                raise HTTPException(
                    422, f"Значение ниже минимума ({field_info.ge})")
            if hasattr(field_info, "le") and field_info.le is not None and value > field_info.le:
                raise HTTPException(
                    422, f"Значение выше максимума ({field_info.le})")
    ...
    if metadata.get("multiplier"):
        display_value = updated_value * \
            metadata["multiplier"] if isinstance(
                updated_value, (int, float)) else value
