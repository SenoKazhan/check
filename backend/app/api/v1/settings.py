...
async def list_settings(
    ...
    for key in settings.get_dynamic_settings_keys():
        ...
        setting = SettingResponse(
            ...
            min=getattr(settings.__fields__.get(key), "ge", None) if hasattr(settings, "__fields__") else None,
            max=getattr(settings.__fields__.get(key), "le", None) if hasattr(settings, "__fields__") else None,
            step=meta.get("step"),
        )
...
async def update_setting(
    ...
    :param key: Идентификатор настройки
    :param payload: Новое значение + опциональная причина изменения
    """
    # Валидация ключа
    if key not in settings.get_dynamic_settings_keys():
        raise HTTPException(
    ...
    if expected_type == "float" and isinstance(value, (int, float)):
        field_info = getattr(settings.__fields__.get(key), "field_info", None) if hasattr(settings, "__fields__") else None
        if field_info:
            if hasattr(field_info, "ge") and field_info.ge is not None and value < field_info.ge:
                raise HTTPException(422, f"Значение ниже минимума ({field_info.ge})")
            if hasattr(field_info, "le") and field_info.le is not None and value > field_info.le:
                raise HTTPException(422, f"Значение выше максимума ({field_info.le})")
    ...
    logger.info(
        ...
        key, value, current_user["id"], payload.change_reason or "не указано"
    )
