    ...
    try:
        with get_database_connection() as conn:
            with conn.cursor() as cursor:
                ...
                logger.info("✓ %s %s: операция выполнена", operation_name, log_context)
                return True
...
def create_measurement_record(
    ...
    with get_database_connection() as conn:
        with conn.cursor() as cursor:
            ...
            logger.info(
                "✓ Создана запись измерения #%d (статус: PENDING)", measurement_id
            )
...
def update_measurement_status(measurement_id: int, status: MeasurementStatus) -> bool:
    ...
    return execute_db_update(
        query, params, operation_name="Обновление статуса", measurement_id=measurement_id
    )
...
def finalize_measurement(
    ...
    return execute_db_update(
        query, params, operation_name="Финализация измерения", measurement_id=measurement_id
    )
...
def _fetch_setting(conn, key: str, default: any) -> any:
    ...
    for file_path in file_paths:
        ...
        if path.exists():
            try:
                ...
                logger.warning("Не удалось удалить файл %s: %s", file_path, error)
...
def aggregate_dimensions(view_results: Dict[str, Dict[str, float]]) -> Dict[str, float]:
    """
...
def process_measurement_task(
    ...
    try:
        ...
        if len(image_paths) != len(REQUIRED_VIEWS):
            raise ValueError(
                f"Требуется ровно {len(REQUIRED_VIEWS)} изображения: {', '.join(REQUIRED_VIEWS)}"
            )
        ...
        # === Шаг 2.5: Загрузка динамических настроек (Выравнивание и пороги) ===
        conn_cfg = _get_sync_db_connection()
        enable_alignment = _fetch_setting(conn_cfg, "cv_enable_alignment", True)
        reference_angle = _fetch_setting(conn_cfg, "cv_alignment_reference_angle", 0.0)
        conn_cfg.close()

        logger.info(
            "Конфигурация CV: alignment=%s, angle=%.1f°", enable_alignment, reference_angle
        )
        ...
        # === Шаг 6: Сохранение финальных результатов ===
        finalize_measurement(
            measurement_id=measurement_id,
            dimensions=dimensions,
            status=final_status,
            verification_metrics={"delta_pct": delta_pct, "verified_ok": verified_ok},
        )
        ...
        logger.error(
            ...
            error,
            exc_info=True,
            extra={"measurement_id": measurement_id, "user_id": user_id},
        )
